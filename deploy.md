...
`http://betaapi.dictybase.local` and `https://betaauth.dictybase.local` to
various HTTP services. So, `https://betatest.dictybase.local` will be mapped to
frontend, `https://betaapi.dictybase.local` will be mapped to various services
such as users,contents and `https://betatauth.dictybase.local` will be mapped
to auth service.
...
Run any one of the above `https` url in your browser and confirm the security exception so that
